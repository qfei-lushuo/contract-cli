...
func TestCheckCachedReadsLocalCacheOnly(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), "update-check.json")
	if notice := CheckCached(path, "0.1.0-beta.1", DefaultPackageName); notice != nil {
		t.Fatalf("CheckCached() without cache = %+v, want nil", notice)
	}
	...
	notice := CheckCached(path, "0.1.0-beta.1", DefaultPackageName)
	if notice == nil {
		t.Fatal("CheckCached() = nil, want notice")
	}
	if notice.Current != "0.1.0-beta.1" || notice.Latest != "0.1.0-beta.2" {
		t.Fatalf("notice versions = %+v", notice)
	}
...
func TestRefreshCacheSkipsFreshCache(t *testing.T) {
	...
	requests := 0
	err := RefreshCache(context.Background(), path, Options{
		HTTPClient: &http.Client{
			Transport: roundTripFunc(func(*http.Request) (*http.Response, error) {
				requests++
				return jsonHTTPClient(`{"dist-tags":{"beta":"0.1.0-beta.2"}}`).Transport.RoundTrip(&http.Request{})
			}),
		},
		RegistryURL:    "https://registry.test/@qfeius%2fcontract-cli",
		CurrentVersion: "0.1.0-beta.1",
	...
	if requests != 0 {
		t.Fatalf("RefreshCache() requests = %d, want 0 for fresh cache", requests)
	}
}

func TestRefreshCacheWritesStaleCache(t *testing.T) {
	t.Parallel()
	...
	if err := RefreshCache(context.Background(), path, Options{
		HTTPClient:     jsonHTTPClient(`{"dist-tags":{"beta":"0.1.0-beta.2"}}`),
		RegistryURL:    "https://registry.test/@qfeius%2fcontract-cli",
		CurrentVersion: "0.1.0-beta.1",
		Channel:        "beta",
		Now:            fixedNow,
	...
	if !ok || cache.LatestVersion != "0.1.0-beta.2" || !cache.UpdateAvailable {
		t.Fatalf("cache after RefreshCache() = %+v, ok=%v", cache, ok)
	}
}
