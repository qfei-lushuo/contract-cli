	...
	testCases := []struct {
		...
		},
		{
			...
			fragments: []string{
				...
				"mdm vendor create/update` 必须传 `--user-id`",
				"请求体不要包含后端生成的 `vendor` 编码",
				"请求体必须包含后端返回的 `id` 和 `vendor` 编码",
			},
		},
		{
		...
		},
		{
			...
			fragments: []string{
				...
				"rule table *",
				"例外：`mdm vendor create/update` 与 `mdm legal create/update` 写接口会本地要求 `--user-id`",
			},
		},
	}
