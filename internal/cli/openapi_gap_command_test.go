...
func TestOpenAPIGapCommandsUseExpectedEndpointsAsBot(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			wantQuery:  map[string]string{"source_currency": "CNY", "target_currency": "USD", "effective_date": "2026-06-01", "user_id_type": "user_id"},
		},
		{
		...
		},
		{
			...
			args:       []string{"mdm", "vendor", "create", "--profile", "contract", "--data", `{"vendor":"V0001"}`},
			wantMethod: http.MethodPost,
			wantPath:   "/open-apis/mdm/v1/vendors",
			wantQuery:  map[string]string{"user_id_type": "user_id"},
			wantBody:   `{"vendor":"V0001"}`,
		},
		{
			name:       "mdm vendor update",
			args:       []string{"mdm", "vendor", "update", "vendor-1", "--profile", "contract", "--data", `{"vendor":"V0001"}`},
			wantMethod: http.MethodPut,
		...
		},
		{
			...
			args:       []string{"mdm", "legal", "create", "--profile", "contract", "--data", `{"legal_entity":"L0001"}`},
			wantMethod: http.MethodPost,
			wantPath:   "/open-apis/mdm/v1/legal_entities",
			wantQuery:  map[string]string{"user_id_type": "user_id"},
			wantBody:   `{"legal_entity":"L0001"}`,
		},
		{
			name:       "mdm legal update",
			args:       []string{"mdm", "legal", "update", "legal-1", "--profile", "contract", "--data", `{"legal_entity":"L0001"}`},
			wantMethod: http.MethodPut,
