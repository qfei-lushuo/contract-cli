...
func TestMCPCommandsUseUserIdentityAndExpectedEndpoints(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			args:         []string{"contract", "sync-user-groups", "--profile", "contract", "--user-id", "ou_app_owner"},
			wantMethod:   http.MethodPost,
			wantPath:     "/open-apis/contract/v1/contracts/user-groups/sync",
			wantQuery:    map[string]string{"user_id": "ou_app_owner"},
			wantAuth:     "Bearer app-token",
		...
		},
		{
			...
			args:         []string{"contract", "template", "list", "--profile", "contract", "--category-number", "CAT-1", "--page-size", "20", "--page-token", "next", "--user-id", "ou_app_owner", "--user-id-type", "employee_id"},
			wantMethod:   http.MethodGet,
			wantPath:     "/open-apis/contract/v1/templates",
			wantQuery:    map[string]string{"category_number": "CAT-1", "page_size": "20", "page_token": "next", "user_id": "ou_app_owner", "user_id_type": "employee_id"},
			wantAuth:     "Bearer app-token",
		...
		},
		{
			...
			args:         []string{"contract", "template", "get", "tpl-1", "--profile", "contract", "--user-id", "ou_app_owner", "--user-id-type", "employee_id"},
			wantMethod:   http.MethodGet,
			wantPath:     "/open-apis/contract/v1/templates/tpl-1",
			wantQuery:    map[string]string{"user_id": "ou_app_owner", "user_id_type": "employee_id"},
			wantAuth:     "Bearer app-token",
...
func TestLegacyMCPCommandNamesAreRejected(t *testing.T) {
	...
	for _, args := range testCases {
		...
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			t.Parallel()
			// Each CLI invocation owns its state and output buffers.
			app := cli.New(cli.Options{
				Stdout: &bytes.Buffer{},
				Stderr: &bytes.Buffer{},
				Store:  config.NewStore(t.TempDir()),
