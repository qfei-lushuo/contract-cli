...
- 先通过 `contract-cli version` 确认安装包固定的环境，按包选择 profile。prod 使用 `contract`，dev/test/blue 分别使用对应的 `contract-<env>`。安装对应的 dev、test 或 blue 包后，分别使用 `contract-dev`、`contract-test`、`contract-blue`；每个包只支持构建指定的一个环境。
- 所有非生产授权和业务命令必须显式指定 profile：dev 必须显式带 `--profile contract-dev`，test 和 blue 分别带 `--profile contract-test`、`--profile contract-blue`，包括替换下游技能示例中的 `--profile contract`。禁止复用生产凭证，禁止失败后回退 prod。初始化和单次授权规则见 Auth 技能；不能因存在旧 profile 自动切换环境。
