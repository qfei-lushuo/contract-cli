...
- `auth init` 返回后严格执行授权 Skill 的展示契约：WorkBuddy 使用 `show_widget` 内联展示二维码，AgentKit 继续使用 `qr_code_path`；豆包普通工作任务必须把 `qr_code_path` 作为图片附件或图片产物交付。交付成功的同一条最终回复必须同时提供可点击授权链接、二维码和过期时间，未实际出现图片时不得声称已经展示二维码。
