...
| --page-size | $query.page_size | integer | 可选 | CLI 支持；官方规格未列出该 query。 |
...
- 官方规格未列出分页 query；当前 CLI 支持并发送 `page_size` / `page_token`。

## 示例

```bash
contract-cli rule table row list --product-id contract --group-id approve_matrix --table-id <table-id> --profile contract --as app
```
