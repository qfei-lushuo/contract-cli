# mdm fixed-exchange-rate update Parameters

本页专用于 `contract-cli mdm fixed-exchange-rate update`。

- 接口：`PUT /open-apis/mdm/v1/fixed_exchange_rate`
- 身份：仅 `app`
- 请求体：JSON 必填，`--input-file` 与 `--data` 二选一
- 官方 OpenAPI：[更新固定汇率](https://docs.qfei.cn/373497572e0.md)
...
| JSON 路径 | 类型 | 必填性 | 说明 |
| --- | --- | --- | --- |
| source_currency | string | 可选 | 示例值："USD" |
| target_currency | string | 可选 | 示例值："CNY" |
| effective_date | string | 可选 | 示例值："2022-02-13" |
| exchange_rate | string | 可选 | 示例值："0.1720000000" |
| status | integer | 可选 | 示例值：1默认值：1 |

## 枚举与约束

- 以原始币种、目标币种、生效日期的组合作为唯一键设定固定汇率，如果指定条件下无汇率值，则新增一条汇率值，否则更新当前条件下汇率值。
- `status`（integer，可选）：示例值：1默认值：1

## 示例

```bash
contract-cli mdm fixed-exchange-rate update --input-file request.json --profile contract --as app
```
...
- 官方规格路径：`PUT /open-apis/mdm/v1/fixed_exchange_rate`
