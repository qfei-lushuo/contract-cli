# contract cooperation file download Parameters

本页专用于 `contract-cli contract cooperation file download`。

- 接口：`GET /open-apis/contract/v1/contracts/cooperation/{file_id}/download_file`
- 身份：仅 `app`
- 请求体：官方接口不要求 JSON 请求体
- 官方 OpenAPI：[下载协商文件](https://docs.qfei.cn/465843711e0.md)
...
| 参数 | 请求位置 | 类型 | 必填性 | 说明 |
| --- | --- | --- | --- | --- |
| <file-id> | $path.file_id | string | 必填（服务端） | 协商文件id |
...
```bash
contract-cli contract cooperation file download <file-id> --profile contract --as app
```

## 来源差异说明

- 官方规格路径：`GET /open-apis/contract/v1/contracts/cooperation/{cooperation_file_id}/download_file`
- CLI 路径表达：`GET /open-apis/contract/v1/contracts/cooperation/{file_id}/download_file`；示例 ID 或占位名已统一为 CLI 名称。
