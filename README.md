...
默认使用 `contract` profile 和 `prod` 环境。当前分支支持独立的 `contract-dev`、`contract-test`、`contract-blue` profile 进行提测，必须显式选择且分别授权，详见 [多环境提测说明](docs/environment-testing.md)。WorkBuddy 更新 Skills 后必须完全退出并重新启动，然后新建任务；已有任务不会热加载新 Skill。
